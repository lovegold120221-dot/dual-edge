...
- `lib/main.dart` — entry point: `Firebase.initializeApp` + `AppController`
- `lib/app.dart` — `DualTranslateApp` with routing: Loading → `ConfigurationScreen` → `AuthScreen` → `TranslatorScreen`
...
- `lib/services/` — `LiveApiService` (Gemini WebSocket), `LiveAudioService` (native duplex PCM), `DuplexEchoGuard` (echo cancellation), `FirebaseService`, `PdfExportService`, `PreferenceStore`
...
- `tool/` — smoke scripts: `smoke_gemini_live.dart`, `smoke_firebase_rest.dart`, `smoke_realtime_transcription.dart`
...
- Never commit real API keys, Firebase service files, Android signing files, keystores, or client secrets
...
- Firebase comes from `firebase_options.dart` plus the gitignored Google services files; not passed through dart-defines
...
- `dart run tool/smoke_firebase_rest.dart <json>` — probes Auth + Realtime Database REST; expects `FIREBASE_API_KEY` / `FIREBASE_DATABASE_URL`; this is a leftover connectivity check, not how the running app talks to Firebase